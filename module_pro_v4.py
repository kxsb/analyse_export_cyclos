...
class UIManager:
    ...
    def importer(self):
        ...
        imported_files = []  # Liste des fichiers correctement importés

        for file_path in file_paths:
            df = self.data_manager.load_file(file_path, self.root)
            if df is not None:
                self.data_manager.add_data(df)
                imported_files.append(file_path)  # Ajouter à la liste des fichiers traités

        # Afficher les fichiers importés directement dans l'interface
        self.display_imported_files(imported_files)

        # Mise à jour des statistiques après import
